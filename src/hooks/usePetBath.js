...
export function usePetBath(){
 ...
 const act=useCallback(async(recipient,action)=>{
  ...
  try{const {data,error}=await getSupabase().rpc('heart_pet_bath',args).abortSignal(AbortSignal.timeout(15000));if(error)throw error;pending.current=null;publish(data,seq);}
  catch{if(alive.current)setError('ส่งไม่สำเร็จ กดช่วยอีกครั้งเพื่อลองส่งรายการเดิม');}
