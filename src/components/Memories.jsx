...
export function PersonalBook({onClose}) {
 const {donations,replayCelebration,lastSyncedAt}=useCollection();const [own,setOwn]=useState(null),[ownError,setOwnError]=useState('');
 useEffect(()=>{let alive=true;(async()=>{try{const ids=ownedOrders(),all=[];for(let i=0;i<ids.length;i+=100){let cursor=null;do{const page=await fetchMemories({ids:ids.slice(i,i+100),cursor});if(!alive)return;all.push(...page);const last=page.at(-1);cursor=page.length===24?{stamp:last.createdAt,id:last.id}:null;}while(cursor);}if(alive){setOwn(all);setOwnError('');}}catch{if(alive)setOwnError('ยังโหลดประวัติส่วนตัวไม่ครบ กรุณาเปิดอีกครั้ง');}})();return()=>{alive=false;};},[lastSyncedAt]);
 const {history,badges,hearts}=getPersonalMemories(own||donations.filter(d=>ownedOrders().includes(d.id)));
...
export function TogetherCelebration({event,onDone,hatched={rose:true,praew:true}}) {
 ...
 return <Modal wide title="A little magic, together" onClose={onDone}>
  <div className={`together-scene ${arrived?'friends-arrived':''}`}>
   ...
   {hatched.rose&&<div className="celebration-friend friend-rose"><FamiliarArt kind="rabbit" awake ribbon charm/><small>Rose’s rabbit</small></div>}
   {hatched.praew&&<div className="celebration-friend friend-praew"><FamiliarArt kind="alpaca" awake ribbon charm/><small>Praew’s alpaca</small></div>}
