...
export default function AdminPanel({scope='all',settings=false}){
 const [data,setData]=useState(null),[error,setError]=useState(''),[errorTarget,setErrorTarget]=useState('global'),[busy,setBusy]=useState(false),[username,setUsername]=useState(''),[password,setPassword]=useState(''),[filter,setFilter]=useState('pending'),[search,setSearch]=useState('');
 const refresh=async()=>setData(await (settings?getAdminSettings():getAdminState(scope)));
 async function run(fn,target='global'){if(busy)return;setBusy(true);setError('');setErrorTarget(target);try{await fn();await refresh();}catch(e){setError(e.message);if([401,403].includes(e.status))setData(null);}finally{setBusy(false);}}
 ...
 const visibleOrders=orders.filter(o=>(filter==='all'||o.status===filter)&&(!search.trim()||[o.form.supporterName,o.form.message].some(value=>(value||'').toLowerCase().includes(search.trim().toLowerCase())))).slice().reverse();
 ...
 {error&&(!data||errorTarget==='global')&&<p className="error" role="alert">{error}</p>}
 ...
 <ExternalDonationForm key={scope} scope={scope} actor={data.actor} busy={busy||!data.readiness.enabled} error={errorTarget==='external'?error:''} onSave={(form,id)=>run(()=>recordExternalDonation(form,id,scope),'external')}/>
 <section className="admin-queue">
 <div className="admin-queue-heading"><div><h2>รายการโดเนท</h2><p>รอตรวจ {orders.filter(o=>o.status==='pending').length} รายการ · {name(scope)}</p></div><button className="secondary admin-refresh" disabled={busy} onClick={()=>run(async()=>{})}>{busy?'กำลังอัปเดต…':'↻ รีเฟรช'}</button></div>
 <div className="admin-toolbar"><div className="admin-status-tabs" role="group" aria-label="สถานะรายการ">{[['pending','รอตรวจ'],['approved','อนุมัติแล้ว'],['rejected','ปฏิเสธแล้ว'],['all','ทั้งหมด']].map(([value,label])=><button key={value} type="button" aria-pressed={filter===value} onClick={()=>setFilter(value)}>{label}<span>{orders.filter(o=>value==='all'||o.status===value).length}</span></button>)}</div><label className="admin-search"><span>ค้นหารายการ</span><input type="search" placeholder="ค้นหาชื่อหรือข้อความ" value={search} onChange={e=>setSearch(e.target.value)}/></label></div>
 {!visibleOrders.length&&<div className="admin-empty"><b>{search.trim()?'ไม่พบรายการที่ค้นหา':filter==='pending'?'ตรวจครบแล้ว ไม่มีรายการรอตรวจ':'ยังไม่มีรายการในหมวดนี้'}</b><p>{search.trim()?'ลองเปลี่ยนชื่อหรือข้อความที่ค้นหา':'รายการใหม่จะแสดงที่นี่เมื่ออัปเดตข้อมูล'}</p></div>}
 {visibleOrders.map(o=><AdminOrder key={o.id} scope={scope} o={o} busy={busy} enabled={data.readiness.enabled} error={errorTarget===o.id?error:''} onReview={(decision,evidence)=>run(()=>reviewOrder(o.id,decision,evidence,scope),o.id)} onDelete={()=>run(()=>deleteApprovedDonation(o.id,scope),o.id)}/>)}
 </section>
 </>}
 </>}
 </main>;
}
export function AdminOrder({o,busy,enabled,onReview,onDelete,scope,error}){
 const [slipURL,setSlipURL]=useState(''),[slipError,setSlipError]=useState(''),[reloadSlip,setReloadSlip]=useState(0);
 ...
 return <article className={'admin-order admin-review-card'+(!o.hasSlip?' admin-without-slip':'')}>
 <header className="admin-order-heading"><span className={'admin-order-status status-'+o.status}>{o.status==='pending'?'รอตรวจสอบ':o.status==='approved'?'อนุมัติแล้ว':'ปฏิเสธแล้ว'}</span><span>ส่งหัวใจให้{name(o.form.recipient)}</span></header>
 ...
 <div className="admin-order-content">{error&&<p className="error admin-action-error" role="alert">{error}</p>}<dl className="admin-donation-summary"><div><dt>ชื่อ</dt><dd>{o.form.supporterName}</dd></div><div><dt>ข้อความ</dt><dd style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{o.form.message||'—'}</dd></div><div><dt>หัวใจ</dt><dd>{o.form.quantity} × {heart?.name||o.form.heartType} · {name(o.form.recipient)}</dd></div><div className="admin-order-amount"><dt>ยอดโดเนท</dt><dd>{Number(o.amount).toLocaleString()} <small>THB</small></dd></div></dl>
 ...
 {o.status==='rejected'&&<div className="admin-actions"><button className="primary" disabled={busy||!enabled||!slipURL||!!slipError} onClick={()=>onReview('approve',{confirmed:true,reviewMethod:'manual'})}>อนุมัติ</button><button className="secondary admin-reject" disabled={busy} onClick={onDelete}>ลบรายการ</button></div>}
 {o.status==='approved'&&<button className="secondary admin-reject" disabled={busy} onClick={onDelete}>ลบหัวใจที่อนุมัติแล้ว</button>}
 </div>
 </article>;
}
