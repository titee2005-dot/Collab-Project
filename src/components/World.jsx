...
import {representativeHearts} from '../services/heartMemories';
...
export function Room({character,onCollect,onCabinet,active,onStory,hatched=false}){
 const {stats,arrival,notice,donations,evolution}=useCollection();const {id,name}=character,s=stats[id]; const [previewId,setPreviewId]=useState(null);const preview=collectibleItems[id].find(i=>i.id===previewId);const previewLocked=preview && preview.at>s.total;
 const next=collectibleItems[id].find(i=>i.at>s.total);
 const latest=donations.filter(d=>d.recipient===id&&!d.id.startsWith('demo-')).at(-1); const memories=representativeHearts(donations,id);
 return <section className={`room ${id} ${active?'mobile-active':''} ${notice?.landed&&notice?.recipient===id?'receiving':''}`} aria-label={`${name}'s collection`}>
  ...
  <div className="room-stage" data-jar={id}>
   <RoomEnvironment recipient={id} total={s.total} sharedTotal={stats.total} evolution={evolution}/>
   ...
   <div className="jar-wrap"><svg className="jar-glass" viewBox="0 0 300 340" aria-hidden="true"><defs><linearGradient id={`glass-${id}`} x2="1" y2=".7"><stop stopColor="#fff5ef" stopOpacity=".8"/><stop offset=".45" stopColor={id==='rose'?'#eac9df':'#f3d5bb'} stopOpacity=".22"/><stop offset="1" stopColor="#fff3e7" stopOpacity=".8"/></linearGradient></defs><path d="M110 62Q75 80 51 126C-14 237 71 313 150 327C229 313 314 237 249 126Q225 80 190 62Z" fill={`url(#glass-${id})`} stroke={id==='rose'?'#bd8ca9':'#c8a27a'} strokeWidth="2.5"/><path d="M97 89Q39 133 49 200M71 245q14 27 40 38" fill="none" stroke="#fff9ef" strokeWidth="8" strokeLinecap="round" opacity=".85"/><ellipse cx="150" cy="64" rx="46" ry="11" fill="#ead1c7" stroke="#b98b85" strokeWidth="2"/><rect x="105" y="44" width="90" height="20" rx="8" fill="#ecd5b6" stroke="#b89b76" strokeWidth="2"/><path d="M113 49H187M117 57H183" stroke="#c4a179"/><path d="M137 41q-13-20 13-29 26 9 13 29Z" fill="#ead7bb" stroke="#b89b85" strokeWidth="2"/></svg>
    <div className="jar-hearts">{memories.map(({id:key,memory},i)=>{const row=Math.floor(i/3),col=i%3;return <button key={key} className="jar-memory" onClick={()=>onStory(memory)} aria-label={'Read '+memory.supporterName+' · '+memory.quantity+' hearts'} style={{left:(i===9?40:12+col*26)+'%',bottom:(12+row*20)+'%',transform:'rotate('+((i*29)%30-15)+'deg)'}}><Heart color={heartTypes.find(h=>h.id===memory.heartType).color}/></button>;})}</div>
   ...
   {hatched&&<Familiar recipient={id} total={s.total} celebrating={arrival?.recipient===id || notice?.recipient===id} previewCharm={previewLocked && preview.kind==='familiar-charm'}/>}<div className="shelf"/><span className="shelf-label">{id==='rose'?'THE MOONLIGHT ROOM':'THE SUNLIGHT ROOM'}</span>
