...
import {Search,LockKeyhole,ArrowUpRight,Download,Share2,Check} from 'lucide-react';
...
import {getRecentDonations,searchSupporters} from '../services/donationService';
export function CollectionBook({onClose}){
 const {stats}=useCollection();const [tab,setTab]=useState('hearts');
 return <Modal wide title="The collection book" onClose={onClose}><p className="muted">Little treasures. A growing story, written together.</p><div className="tabs">{['hearts','rose','praew','shared','memories'].map(t=><button key={t} aria-pressed={tab===t} className={tab===t?'active':''} onClick={()=>setTab(t)}>{t==='memories'?'Heart Memories':t==='hearts'?'Heart Collection':t==='shared'?'Shared Magic':`${t==='rose'?'Rose':'Praew'} Items`}</button>)}</div>
 {(tab==='rose'||tab==='praew')&&<p className="unlock-points-summary">{stats[tab].total.toLocaleString()} หัวใจ <b>{stats[tab].points.toLocaleString()} คะแนนปลดล็อก</b></p>}
 {tab==='memories'?<MemoryExplorer/>:tab==='hearts'?<div className="book-grid">{heartTypes.map(h=><article key={h.id}><Heart color={h.color}/><h3>{h.name}</h3><p>{h.meaning}</p><small>+{h.unlockPoints} คะแนนปลดล็อก / ดวง</small><span>{stats.rose.types[h.id]+stats.praew.types[h.id]} collected</span></article>)}{specialHearts.map(h=>{const open=stats.points>=h.unlock;return <article className={open?'discovered':'locked'} key={h.id}><Heart color={h.color}/>{!open&&<LockKeyhole className="lock" size={20}/>}<h3>{open?h.name:'???'}</h3><p>{open?h.meaning:`ปลดล็อกที่ ${h.unlock.toLocaleString()} คะแนน`}</p><span>{open?'Discovered together':`${Math.max(0,h.unlock-stats.points).toLocaleString()} คะแนนเพื่อปลดล็อก`}</span></article>;})}</div>:<div className="cabinet-grid">{(tab==='shared'?milestones:collectibleItems[tab]).map((item,i)=>{const open=(tab==='shared'?stats.total:stats[tab].points)>=item.at;return <article key={item.name} className={open?'':'locked'}>{tab==='shared'?<Prop kind={item.kind||['wand','rabbit','crystal','book','crown'][i]}/>:<ItemArtwork item={item}/>}<div><h3>{item.name}</h3><p>{open?'Unlocked':`ปลดล็อกที่ ${item.at.toLocaleString()} ${tab==='shared'?'หัวใจ':'คะแนน'}`}</p></div>{open?<Check size={18}/>:<LockKeyhole size={16}/>}</article>;})}</div>}
 <p className="book-foot">ไอเทมของแต่ละห้องและหัวใจพิเศษใช้คะแนนปลดล็อก ส่วน Shared Magic ใช้จำนวนหัวใจจริง</p></Modal>
}
...
export function Unlock({heart,onClose,onCollection}){return <Modal title="New Heart Discovered" onClose={onClose}><div className="unlock-art"><span>✧</span><Heart color={heart.color}/><span>✧</span></div><div className="unlock-copy"><span className="eyebrow">A COMMUNITY DISCOVERY</span><h3>{heart.name}</h3><p>{heart.meaning}</p><small>ปลดล็อกร่วมกันที่ {heart.unlock.toLocaleString()} คะแนน</small></div><button className="primary full" onClick={onCollection}>View Collection</button><button className="text-button full" onClick={onClose}>Skip celebration</button></Modal>}
