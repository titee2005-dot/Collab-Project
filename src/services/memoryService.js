...
export function getPersonalMemories(donations){
 const allHistory=donations.filter(d=>!d.id.startsWith('demo-')).sort((a,b)=>Date.parse(a.createdAt)-Date.parse(b.createdAt));
 const history=allHistory.filter(d=>!['admin-adjustment','external-payment'].includes(d.source));
 let total=donations.filter(d=>d.id.startsWith('demo-')).reduce((sum,d)=>sum+d.quantity,0);
 const events=allHistory.flatMap(d=>{const crossed=d.memoryMilestones??crossedMilestones(total,total+d.quantity).map(m=>m.at);total+=d.quantity;return (['admin-adjustment','external-payment'].includes(d.source)?[]:crossed).map(at=>({at,donationId:d.id,createdAt:d.createdAt}));});
