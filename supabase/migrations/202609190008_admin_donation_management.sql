...
begin
 ...
 if p_action='mode' then
  ...
  if o->>'status'<>'approved' then raise exception 'ลบได้เฉพาะรายการที่อนุมัติแล้ว'; end if;
  reason:='แอดมินลบรายการที่อนุมัติและคืนยอดหัวใจ';
...
create or replace function heart_private.heart_api_access(p_action text,p_payload jsonb default '{}',p_actor uuid default null)
returns jsonb language plpgsql security definer set search_path='' as $$
declare member heart_private.admins%rowtype; wanted text; target text; result jsonb;
begin
 if p_action in ('authorize','state','settings','mode','policy','review','delete','external','slip') then
  ...
  if p_action in ('review','delete','slip') then
...
create or replace function public.heart_api(p_action text,p_payload jsonb default '{}',p_actor uuid default null)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if p_payload->>'reviewerHash' is not null then
  ...
  if p_action not in ('state','review','delete','external','slip') then raise exception using errcode='42501',message='หน้านี้สำหรับเจ้าของระบบเท่านั้น'; end if;
...
revoke all on function heart_private.heart_api_base(text,jsonb,uuid),heart_private.heart_api_access(text,jsonb,uuid) from public,anon,authenticated,service_role;
