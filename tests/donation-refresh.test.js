...
test('refresh retains all queued memories beyond the RPC 100-ID limit without multiplying totals',async()=>{
 ...
 assert.deepEqual(calls.map(c=>c.length),[100,100,50]);assert.equal(rows.length,251);assert.equal(new Set(rows.map(d=>d.id)).size,251);assert.equal(rows.collectionStats.total,251);
});
